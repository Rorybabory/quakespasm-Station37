...
Mins and maxs enclose the entire area swept by the moveSV_Move
====================
*/
...
====================
*/
void SV_ClipToLinksUse(areanode_t* node, moveclip_t* clip)
{
	link_t* l, * next;
	edict_t* touch;
	trace_t		trace;
	...
	if (clip->boxmaxs[node->axis] > node->dist)
		SV_ClipToLinksUse(node->children[0], clip);
	if (clip->boxmins[node->axis] < node->dist)
		SV_ClipToLinksUse(node->children[1], clip);
}
...
==================
*/
trace_t SV_MoveUse(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int type, edict_t* passedict)
{
	moveclip_t	clip;
	int			i;
	...
	// clip to entities
	SV_ClipToLinksUse(sv_areanodes, &clip);
