...
float easing(float x) {
	const float c5 = (2.0f * 3.1515f) / 4.5f;

	if (x == 0) { return 0.0f; }
	if (x == 1) { return 1.0f; }

	if (x < 0.5) {
		return -(pow(2, 20.0f * x - 10.0f) * sin((20.0f * x - 11.125) * c5)) / 2.0f;
	} else {
		return -(pow(2,-20.0f * x + 10.0f) * sin((20.0f * x - 11.125) * c5)) / 2.0f + 1.0f;
	}
}
...
void R_DrawViewModel (void)
{
	...
	if (!currententity->model)
		return;
	if (Inventory_GetChange() == 1) {
		currententity->viewmodellerp = 0;
	}
	//printf("%f %f %f\n", currententity->previousangles[0], currententity->previousangles[1], currententity->previousangles[2]);
	//currententity->angles[0] = -90;

	currententity->lastmodel = currententity->model;
	if (currententity->viewmodellerp < 1.0) {
		currententity->viewmodellerp += ui_deltatime*2;
	}
	currententity->angles[0] += -45.0f + easing2(currententity->viewmodellerp)*45.0f;
...
/*
================
R_RenderSceneLighting
================
*/
void R_RenderSceneLighting(void)
{
...
void R_RenderDepthView(void) {
	...
	R_DrawTextureChains_Drawflat(cl.worldmodel, chain_world);
