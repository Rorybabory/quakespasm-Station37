...
static void Host_Changelevel_f (void)
{
	...
	//Inventory_Clear();
	for (int i = 0; i < 18; i++) {
		start_inv_slots[i] = inv_slots[i];
	}
	inv_startnumslots = inv_numslots;
	strcpy(inv_lastname, "");
	if (Cmd_Argc() != 2)
	{
...
static void Host_Restart_f (void)
{
	...
	inv_cash = 0;
	inv_numslots = inv_startnumslots;
	for (int i = 0; i < 9; i++) {
		inv_slots[i] = start_inv_slots[i];
	}
...
// this silliness is so we can load 1.06 save files, which have float skill values
	...
	data = COM_ParseIntNewline(data, &inv_numslots);

	data = COM_ParseIntNewline(data, &tutorialflags);
	...
	for (int x = 0; x < inv_numslots; x++) {
		data = COM_ParseStringNewline(data);
		q_strlcpy(inv_slots[x].name, com_token, sizeof(inv_slots[x].name));
		data = COM_ParseIntNewline(data, &inv_slots[x].id);
		data = COM_ParseFloatNewline(data, &inv_slots[x].durability);
...
static void Host_StartItems_f(void) {
	Inventory_AddItem("pistol", 1, 0.6);
	Inventory_AddItem("shotgun", 1, 1);
	Inventory_AddItem("hammer", 1, 1);
	inv_numslots = 9;
	sv_player->v.ammo_cells += 10;
	sv_player->v.ammo_shells += 2;
}
static void Host_EndItems_f(void) {
	Inventory_AddItem("revolver", 1, 0.6);
	Inventory_AddItem("shotgun", 1, 1);
	Inventory_AddItem("super_shotgun", 1, 1);
	Inventory_AddItem("rifle", 1, 1);
	Inventory_AddItem("hammer", 1, 1);
	inv_numslots = 13;
	sv_player->v.ammo_cells += 30;
	sv_player->v.ammo_shells += 15;
	sv_player->v.ammo_nails += 30;
}
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand("startitems", Host_StartItems_f);
	Cmd_AddCommand("enditems", Host_EndItems_f);
