...
================
*/
qpic_t* Draw_PicFromWadRGBA(const char* name)
{
	qpic_t* p;
	glpic_t	gl;
	src_offset_t offset; //johnfitz
...
================
*/
void Draw_CharacterScale(int x, int y, int scale, int num)
{
	if (y <= -8)
		return;			// totally off screen
...
void Draw_StringScale(int x, int y, int scale, const char* str)
{
	if (y <= -8)
		return;			// totally off screen
	...
	{
		if (*str != 32) //don't waste verts on spaces
			Draw_CharacterQuadScale(x, y, scale, *str);
		str++;
		x += scale;
	}
