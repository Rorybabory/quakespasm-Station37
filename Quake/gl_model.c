...
void Mod_LoadAliasModel (qmodel_t *mod, void *buffer)
{
	...
	//unsigned char* data = Image_LoadImage(mod->baseName, &width, &height);
	if (data)
	{
		...
		printf("\nloaded texture with %s path and value %i\n\n", mod->baseName, mod->imgTex);
	}
	else
	{
		printf("model did not have texture at path %s\n", mod->baseName);
		mod->imgTex = -1;
	}
