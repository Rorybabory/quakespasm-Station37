...
same as above, however this is for the use button.


usetraceline (vector1, vector2, tryents)
=================
*/
static void PF_usetraceline(void)
{
	float* v1, * v2;
	trace_t	trace;
	int	nomonsters;
	...
	trace = SV_MoveUse(v1, vec3_origin, vec3_origin, v2, nomonsters, ent);
...
static void PF_EnableKeypad(void) {

	edict_t * ed = G_EDICT(OFS_PARM0);
	char* code = G_STRING(OFS_PARM1);
	...
	int success = (int)G_FLOAT(OFS_PARM2);

	UI_EnableKeypad(i0, i1, i2, i3, success, ed);
}
static void PF_OpenComputer(void) {
	ui_computeropen = true;
	ui_compmode = 0;
	char* computer = G_STRING(OFS_PARM0);
	ui_active_computer = computer;
}

static void PF_SetDialogText(void) {
	dialogbox_text = G_STRING(OFS_PARM0);
	showdialog = 1;
	dialogcounter = strlen(dialogbox_text);
}
static void PF_SetBossHealth(void) {
	bosshealth = G_FLOAT(OFS_PARM0);
	if (bosshealth < 0) {
		drawbosshealth = 0;
	}
	else {
...
static void PF_SetKeypadPointer(void) {
	edict_t* ed = G_EDICT(OFS_PARM0);
	ui_activekeypad = ed;
}
static void PF_UpdateKeypad(void) {
	edict_t* ed = G_EDICT(OFS_PARM0);
	UI_UpdateKeypad(ed);
}
static void PF_GetHeldItemID(void) {
	G_FLOAT(OFS_RETURN) = (float)inv_slots[inv_highlight].id;
}
static void PF_AddCash(void) {
	inv_cash += G_FLOAT(OFS_PARM0);
}
static void PF_GetCash(void) {
	G_FLOAT(OFS_RETURN) = (float)inv_cash;
}

static void PF_DisableKeypad(void) {
	UI_DisableKeypad();
}
static void PF_GetKeypadState(void) {
	G_FLOAT(OFS_RETURN) = (float)((int)ui_keypadSuccess);
}
static void PF_GetStuck(void) {
	int		i, j;
	int		z;
	vec3_t	org;
...
static void PF_Sin(void) {
	G_FLOAT(OFS_RETURN) = (float)sin((double)G_FLOAT(OFS_PARM0));
}
static void PF_GetUse(void) {
	G_FLOAT(OFS_RETURN) = (float) ((int)inv_useClicked);
}
static void PF_SetUse(void) {
	inv_useClicked = (int)G_FLOAT(OFS_PARM0);
}
static void PF_DrawHUDText(void) {
	notes_text = G_STRING(OFS_PARM0);
	notes_count = 0;
}
static void PF_OpenInventory(void) {
	if (inventoryOpen == 1) {
		inventoryOpen = 0;
		IN_Activate();
	}
...
static void PF_getnumitem(void) {
	G_FLOAT(OFS_RETURN) = Inventory_GetNumItem(G_STRING(OFS_PARM0));
}
static void PF_removeitem(void) {
	G_FLOAT(OFS_RETURN) = Inventory_RemoveItem(G_STRING(OFS_PARM0));
}

static void PF_GetFullInventory(void) {
	G_FLOAT(OFS_RETURN) = Inventory_Full();
}
static void PF_GetNumSlots(void) {
	G_FLOAT(OFS_RETURN) = inv_numslots;
}
static void PF_AddSlots(void) {
	inv_numslots += G_FLOAT(OFS_PARM0);
}
static void PF_GetInvChange(void) {
	G_FLOAT(OFS_RETURN) = Inventory_GetChange();
}
static void PF_Sqrt(void) {
	G_FLOAT(OFS_RETURN) = sqrt(G_FLOAT(OFS_PARM0));
}
static void PF_GetSelected(void) {
	G_FLOAT(OFS_RETURN) = (int)ui_selectClicked;
	if (ui_selectClicked == true) {
		ui_selectClicked = false;
	}
}
static void PF_SetInvSlot(void) {
	int value = G_FLOAT(OFS_PARM0);
	inv_highlight = value;
	
	inv_dhighlight = (float)value;

}

static void PF_GetTutorialFlags(void) {
	G_FLOAT(OFS_RETURN) = tutorialflags;
}
static void PF_AddTutorialFlags(void) {
	tutorialflags |= (int)G_FLOAT(OFS_PARM0);
}
...
{
	G_INT(OFS_RETURN) = PR_SetEngineString(inv_slots[inv_highlight].name);
}
...
==============
*/
static void PF_gethasitem(void)
{
	G_FLOAT(OFS_RETURN) = (float) Inventory_HasItem(G_STRING(OFS_PARM0));
}
...
==============
*/
static void PF_getheldnumber(void)
{
	G_INT(OFS_RETURN) = inv_slots[inv_highlight].number;
}
...
static void PF_addItem(void)
{
	...
	int id = (int)G_FLOAT(OFS_PARM1);
	Inventory_AddItem(s, id, G_FLOAT(OFS_PARM2));
	Con_DPrintf("picked up item: %s\n", G_STRING(OFS_PARM0));
}
...
==============
*/
static void PF_removecurrent(void)
{
...
==============
*/
static void PF_subdurability(void)
{
	Inventory_SubtractDurability(G_FLOAT(OFS_PARM0));
}
/*
==============
PF_getdurability
==============
*/
static void PF_getdurability(void)
{
	G_FLOAT(OFS_RETURN) = inv_slots[inv_highlight].durability;
}
...
==============
*/
static void PF_loadfile(void)
{
	
	G_INT(OFS_RETURN) = PR_SetEngineString(UI_NotesLoad(G_STRING(OFS_PARM0)));
	//G_INT(OFS_RETURN) = PR_SetEngineString("why hello there\n");
}
...
==============
*/
static void PF_drawnote(void)
{
	ui_drawnote = true;
	paper_text = G_STRING(OFS_PARM0);
}
...
==============
*/
static void PF_calculatedice(void) {
	G_FLOAT(OFS_RETURN) = (float) parseDiceNotation(G_STRING(OFS_PARM0));
}
/*
==============
2021 re-release
...
static void PF_getdeltatime(void)
{
	G_FLOAT(OFS_RETURN) = ui_deltatime;
}
